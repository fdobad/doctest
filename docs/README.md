...
Currently the FIRE-RES team is working on a firebreak placement feature: Including  landspace clustering (for automatically infering management units) and stochastic network disrupting algorithms!  
...
Using QGIS's plugin manager interface ([ref](https://docs.qgis.org/latest/en/docs/training_manual/qgis_plugins/fetching_plugins.html#basic-fa-configuring-additional-plugin-repositories)):  
...
| 3. Add fire2a's plugin repository & 4. Install the plugin <a id="my-anchor"></a> |
|<img src="img/install_plugin_server.gif"  alt='cannot load image' height=300px >|
| _Finally a new icon ![icon](img/icon.png) on the plugin toolbar and a new plugin menu._ |
|<img src="img/qgis_activate_plugin.gif"  alt='cannot load image' height=400px >|

Now go to install [other useful plugins](#other-useful-plugins), then [usage overview](#broadest-usage-overview).
...
2. Install the plugin requirements (on qgis's python environment)  
2.1 Download [requirements.txt](./requirements.txt)  
2.2.A Bad practice: install directly into python env  
        
        $ pip install --upgrade pip wheel setuptools
        $ pip install -r requirements.txt
        $ pip install --upgrade matplotlib
        
   2.2.B Good practice: Make a python virtual environment (python3-venv required)  
        
        $ python3 -m venv --system-site-packages ~/<your_qgis_env_path>  
        $ source ~/<your_qgis_env_path>/bin/activate
        (your_qgis_env_path) $ pip install --upgrade pip wheel setuptools  
        (your_qgis_env_path) $ pip install -r requirements.txt  
        (your_qgis_env_path) $ pip install --upgrade matplotlib
        
        # launch QGIS:
        (your_qgis_env_path) $ qgis
        
   2.3 Optional tip: Make aliases to activate & launch  
...
## MacOS
TODO  
Installation should be the same than [linux](#linux).  
Compilation too except `apt -> brew`  
...
1. Launch the [python console](https://docs.qgis.org/latest/en/docs/user_manual/plugins/python_console.html) by typing `Ctrl+Alt+P` or clicking on the python icon <img src="img/python-logo-only.png"  alt='cannot load image' height=24px >  
...
1. [Plugin Reloader](https://plugins.qgis.org/plugins/plugin_reloader/#plugin-details) to force hard restart plugins when 🐛bugs🐝🐞 don't go away. Fire2am has a `Restore Defaults` button as a soft restart.  
2. [Serval](https://plugins.qgis.org/plugins/Serval/) to "paint" rasters with your mouse. Useful for creating or editing, for example, the ignition probability layer.  
3. [IPyConsole](https://plugins.qgis.org/plugins/IPyConsole/) is a better python console, with introspection capabilities. `pip install qtconsole` is required.  
...
On the Plugin Menu this plugin dialog launcher is shown selected, also shown available on the Plugin Toolbar ![icon](img/icon.png)  
...
- Windows version supports smaller instances than Linux  
